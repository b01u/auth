...
<script lang="babel">
  ...
  export default{
    ...
    methods: {
       ...
       */
      getModList (systemId) {
        ModMan.getModList({currentPage: 1, pageSize: 9999, systemId: systemId}).then(({ data }) => {
      ...
      launchModule (i, mod) {
        ...
        if (!this.leftGroup[i].sourceList.length) {
          AuthMan.getAuthList({currentPage: 1, pageSize: 9999, moduleId: mod.id}).then(({ data }) => {
       ...
       */
      getSysList() {
        SysMan.getSysList({currentPage: 1, pageSize: 9999}).then(({ data }) => {
